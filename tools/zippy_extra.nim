import zippy/common, zippy/crc, zippy/internal, std/memfiles, std/os,
    std/strutils, std/tables, std/times, std/unicode, zippy/ziparchives_v1,
    zippy
  ...
  of FileRecord:
    if compressionMethod == 0: # No compression
      if record.compressedSize > 0:
        return cast[pointer](src[pos].addr)
    elif compressionMethod == 8: # Deflate
      raise newException(ZippyError, "File is compression, no raw pointer possible")
    else:
